...
| DEV-028 | 9 | 해결 | Special 테스트 어댑터를 제거하고 두 Editor 진단을 공용 살보 API 직접 호출로 바꾸면서 `System.Environment`를 쓰기 위해 추가한 `using System`이 Unity의 `Object`와 C# `object` 이름을 모호하게 만들어 `CS0104`가 발생했다. `using System`을 제거하고 `System.Environment.TickCount`만 완전 수식해 전체 재컴파일 오류 0개를 확인했다. 제품 런타임 코드에는 영향이 없었다. |
