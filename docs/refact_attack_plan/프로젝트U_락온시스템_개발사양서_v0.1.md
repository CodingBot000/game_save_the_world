...
- 공용 풀은 같은 활성 미사일을 중복 대여·반환하지 않는다. `available`이 요청 발수보다 적으면 일부 발사하거나 상한을 늘리지 않고 해당 요청 전체를 발사 전에 `Rejected("PoolCapacityUnavailable")`로 거절한다. 이때 무적과 5초 재사용 대기는 시작하지 않는다.
...
| 5초 후 풀 수량 부족 | 고정 풀 40을 사용했는데도 이전 미사일이 오래 `leased`되어 유효 릴리즈의 요청 전량을 예약하지 못함 | 정상 플레이는 5초 재사용 대기 후 발사를 보장해야 한다. 그럼에도 부족하면 부분 발사나 41번째 생성을 하지 않고 `PoolCapacityUnavailable`로 원자적 실패시키며 화면에 `SHOOT ERROR`를 3초 표시하고 상세 풀 수량을 로그로 남긴다. 이는 정상 게임 흐름이 아니라 검출해야 할 구현·성능 오류다. |
...
| DEV-019 | 5 | 사양대로 처리 확인 | 진단 메뉴에서 5초 재사용 대기만 강제로 종료하고 이전 세대 미사일이 반환되기 전에 20·30발을 연속 요청하자 고정 풀의 가용 수가 부족해 `PoolCapacityUnavailable`이 발생했다. 이는 실제 시간 5초를 기다린 정상 입력이 아니라 검증용 타이머만 앞당긴 호출이었으며, 요청은 첫 발·무적·재사용 대기 전에 원자적으로 거절되고 `SHOOT ERROR`가 정상 표시됐다. 실제 BattleArena에서 7초 대기 후 20발, 이어서 5.2초 후 30발을 요청하면 모두 성공했다. 6단계에서는 DEV-006에 따라 실제 5초 간격, 타겟 소실과 저사양 조건을 계속 검증한다. |
