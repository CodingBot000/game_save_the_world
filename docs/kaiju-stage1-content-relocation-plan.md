...
전역 레거시 경로에 흩어진 Stage 1 Kaiju 전용 모델, 텍스처, 머티리얼, 애니메이션, Animator 구성과 전용 도구를 `Assets/_Project/Content/Bosses/Stage01_Kaiju` 아래로 이전한다.
...
- Stage 1 Kaiju의 모델, 텍스처, 머티리얼, 애니메이션, Controller, Mask, Prefab, Definition은 `Stage01_Kaiju`가 소유한다.
- Stage 2 Saratan의 모든 에셋과 GUID는 계속 `Stage02_Saratan`이 독립적으로 소유한다.
- HP, 피해 처리, 전투 흐름, HUD, 스테이지 로딩처럼 보스에 종속되지 않는 코드는 공용으로 유지한다.
- Kaiju 전용 Animation Driver, State Relay, 디버그 컴포넌트와 Editor 도구는 Stage 1 소유 경로로 이동한다.
- 테스트 코드는 Unity 테스트 관례에 맞춰 `_Project/Tests` 아래에 유지하되 Stage 1 전용 하위 폴더로 정리한다.
- 기존 문서와 Saratan Import Manifest의 과거 원본 경로는 런타임 참조가 아닌 이력 정보이므로 자동 치환하지 않는다.
...
```text
Assets/_Project/Content/Bosses/Stage01_Kaiju/
├── Data/
...
| `Assets/Editor/BuildSaratanStageIntegration.cs` | 전역 Stage 1 경로를 Saratan 금지 의존성으로 검사함 | 새 `Stage01_Kaiju` 경로를 금지 대상으로 추가하고 기존 경로도 회귀 방지용으로 유지 |
...
```text
Assets/_Project/Tests/EditMode/Bosses/Stage01_Kaiju/
...
| 현재 경로 | 목표 경로 |
|---|---|
| `Assets/Invader/Kaiju_001.fbx` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Art/RigA/Models/Kaiju_001.fbx` |
| `Assets/Textures/Invader/Kaiju_*.png` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Art/RigA/Textures/` |
| `Assets/Materials/Invader/Kaiju_*.mat` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Art/RigA/Materials/` |
| `Assets/Animation/Invader/Clips/Kaiju_*.anim` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Art/RigA/Animations/` |
| `Assets/Animation/Invader/KaijuCombat.controller` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Runtime/Animation/Controllers/KaijuCombat.controller` |
| `Assets/Animation/Invader/KaijuUpperBody.mask` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Runtime/Animation/Masks/KaijuUpperBody.mask` |
| `Assets/_Project/Content/Bosses/Stage01_Kaiju/Prefabs/Boss_Stage01_Kaiju.prefab` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Runtime/Prefabs/Boss_Stage01_Kaiju.prefab` |

### 5.2 Kaiju 전용 코드와 도구

| 현재 경로 | 목표 경로 |
|---|---|
| `Assets/_Project/Scripts/Gameplay/KaijuBossAnimationDriver.cs` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Runtime/Scripts/KaijuBossAnimationDriver.cs` |
| `Assets/_Project/Scripts/Gameplay/KaijuCombatStateRelay.cs` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Runtime/Scripts/KaijuCombatStateRelay.cs` |
| `Assets/_Project/Scripts/Debug/KaijuAnimationTester.cs` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Runtime/Debug/KaijuAnimationTester.cs` |
| `Assets/Editor/ApplyKaijuBossVisual.cs` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Editor/Tools/ApplyKaijuBossVisual.cs` |
| `Assets/Editor/KaijuAnimationTestBuilder.cs` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Editor/Tools/KaijuAnimationTestBuilder.cs` |
| `Assets/Editor/KaijuCombatAnimationBuilder.cs` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Editor/Tools/KaijuCombatAnimationBuilder.cs` |
| `Assets/Editor/KaijuCombatAnimationVerification.cs` | `Assets/_Project/Content/Bosses/Stage01_Kaiju/Editor/Tools/KaijuCombatAnimationVerification.cs` |
| `Assets/_Project/Tests/EditMode/KaijuSweepBeamAlignmentTests.cs` | `Assets/_Project/Tests/EditMode/Bosses/Stage01_Kaiju/KaijuSweepBeamAlignmentTests.cs` |
...
3. Kaiju 전용 Editor 도구를 `Stage01_Kaiju/Editor/Tools`로 이동한다.
...
- Stage 2 Prefab과 Definition은 `Stage01_Kaiju` 하위 에셋을 참조하지 않는다.
...
- Stage 1 Kaiju의 활성 에셋과 전용 도구가 `Stage01_Kaiju` 소유 경로에 있다.
...
- 활성 런타임 에셋 21개를 `Stage01_Kaiju/Art/RigA`와 `Runtime/Animation`으로 이동했다.
