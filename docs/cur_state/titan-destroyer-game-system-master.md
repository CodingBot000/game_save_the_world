...
last_verification_scope: "git 4a26b8e의 BattleArena 전투 시작·종료, 플레이어 이동/생존/기관총/락온, 보스 4패턴, HUD 이벤트를 C#·씬 YAML·Prefab·VehiclePlayerStateCatalog에서 정적 교차검증. 현재 씬의 4패턴과 코드 폴백 5패턴 차이, 각 패턴의 실제 투사체 분포·크기 변화·빔 이동 형태, Tail 애니메이션 기반 파편 산탄 준비 시간, 미연결 방송 HUD, 승리/패배 종료 흐름을 문서화. 현재 전투 표의 속도·가속도·거리·폭·피해·HP·배율 단위를 명시. 런타임 PlayMode와 빌드는 이번 검증에서 재실행하지 않음."
partial_review_baseline: "git 4a26b8e 정적 재검토. 사용자 미커밋 BackgroundGroundMortarTrail 머티리얼 변경은 보존하고 검토 범위에서 제외"
implementation_baseline: "git 4a26b8e. BattleArena 씬 직렬화값, 전투 C# 기본값, Player/Boss Prefab, VehiclePlayerStateCatalog 기준. 디버그 런타임 오버라이드 미적용"
...
| 수치 종류 | 문서 단위 |
| --- | --- |
| 이동·투사체·추적 속도 | Unity 월드 단위/초 |
| 가속도 | Unity 월드 단위/초² |
| 거리·폭·반경·위치 진폭 | Unity 월드 단위. `m`, `px`, viewport라고 별도 표기된 값은 해당 단위를 사용 |
| 피해 | 게임 내부 피해량. `피해/발`, `피해/개`, `피해/틱`, `피해/초`처럼 적용 단위를 함께 표기 |
| HP/Hull/Armor | 각 생존 자원의 포인트 |
| 시간 | 초 |
| 각도·회전 속도 | 도(°), 도/초(°/s) |
| 배율·스케일·비율 | 단위 없는 배수. 가능한 경우 어떤 기준값의 배수인지 함께 표기 |
| viewport | 좌하단 `(0,0)`부터 우상단 `(1,1)`까지의 정규화 화면 좌표 |

## 1. 현재 시스템 한눈에 보기

| 영역 | 현재 기준 | 상태 |
| --- | --- | --- |
| 앱 시작 흐름 | Boot → IntroScene → MainMenu → StageStepSelectScene → BattleArena | 구현됨 |
| 전투 형태 | 화면 기준 2D 평면에서 헬기를 이동하며 단일 거대 보스를 상대 | 구현됨 |
| 플레이어 이동 | 좌우·상하 기본 7.2 월드 단위/초, 락온 충전 중 4.32 월드 단위/초 | 구현됨 |
...
| 플레이어 생존 | Hull 100 포인트 / Armor 120 포인트, 아머 자동 수리, 아머 파손 중 Hull 피해 1.25배 | 구현됨 |
| 보스 체력 | 싱글 2,000 HP / 멀티 플레이스홀더 2,800 HP | 구현됨 |
| 활성 보스 패턴 | 파편 산탄, 파편 일제사격, 가속 횡단 빔, 추적 잔류 빔 | 구현됨 |
| 스테이지/난이도 | 5개 지역과 Easy / Normal / Expert / Nightmare 선택값은 HUD 표시에만 사용되고 전투 수치·패턴·배경에는 연동되지 않음 | 불일치 |
...
| 전투 이벤트 HUD | 보스 HP 66.67%/33.33% 대사 HUD는 연결됨. 방송 HUD는 View·프레임만 있고 호출자가 없어 숨김 상태 유지 | 대사 구현됨 / 방송 프로토타입·미연결 |
...
1. 빌드의 첫 씬 `Boot`가 `IntroScene`을 열고, 인트로 입력 후 `MainMenu`로 이동한다.
2. 메인 메뉴의 `Game Start`는 현재 `StageSelectScene`을 거치지 않고 `StageStepSelectScene`을 직접 연다.
3. 지역 페이지와 난이도를 선택하고 스테이지 미리보기 버튼을 누르면 선택값을 저장한 뒤 `BattleArena`에 진입한다.
4. 전투 진입 직후 플레이어 자동 기관총 주기가 시작되고, 보스 첫 패턴은 4초 뒤 시작한다.
5. 플레이어는 화면 평면 이동과 락온 미사일을 사용하면서 보스 Hull을 0으로 만든다.
6. 보스 Hull이 0이면 승리한다. 별도 승리 오버레이는 없고 상태 문구와 `R` 재시작만 제공한다.
7. 플레이어 Hull이 0이면 패배한다. 미션 실패 오버레이의 Retry는 현재 씬을 다시 열고 Quit은 메인 메뉴로 돌아간다.

### 2.2 모드

| 모드 | 보스 체력 | 비고 |
| --- | ---: | --- |
| Single | 2,000 HP | 현재 기본 플레이 경로 |
| MultiPlaceholder | 2,800 HP | 메인 메뉴에서 숨겨진 `M` 키 경로로 진입. 동료 플레이스홀더가 활성화되지만 완성된 멀티플레이 UI·규칙은 없음 |
...
- 현재 메인 메뉴는 `StageStepSelectScene`을 직접 열며, 구형 `StageSelectScene`은 Build Settings와 코드에 남아 있지만 기본 메뉴 흐름에서 호출하지 않는다.
- **현재 확인 결과:** 선택 상태와 UI는 존재하고 BattleArena HUD 디버그 텍스트에 표시된다. 그러나 지역/난이도 선택값을 소비해 보스 체력, 공격력, 패턴, 플레이어 수치, 전투 배경을 바꾸는 런타임 연결은 없다.
...
| --- | --- | --- | --- |
| `MainMenu` | `Assets/Audio/Music/MainMenu/BGM_title.wav` | `MainMenuRoot/Systems/MainMenuMusic` | 씬 활성화 시 자동 시작, 2D, 정규화 볼륨 0.7/1.0, 무한 반복. 전역 OFF이면 즉시 음소거 |
| `BattleArena` | `Assets/Audio/Music/BattleArena/BGM_battle_01.ogg` | `BattleArenaRoot/BattleArenaMusic` | HUD가 동일 소스를 초기화, 2D, 정규화 볼륨 0.7/1.0, 무한 반복. 전역 ON이면 재생하고 OFF이면 음소거 |
...
| 접지 | StageVisualRoot 로컬 Y 0.12~0.20 월드 단위, 런타임 Raycast 없음 |
...
### 2.8 BattleArena 전투 타임라인

| 시점/조건 | 실제 동작 | 상태 |
| --- | --- | --- |
| 씬 진입 | 모드에 따라 보스 HP를 2,000/2,800 포인트로 초기화하고 플레이어 Hull 100 / Armor 120 포인트를 채움 | 구현됨 |
| 전투 시작 직후 | 기관총이 입력 없이 2초 발사 구간으로 시작. 음악/효과음 전역 기본값은 OFF | 구현됨 |
| 2초 | 기관총 2초 휴지 구간 진입. 이후 4초 주기로 반복 | 구현됨 |
| 4초 | 직렬화된 보스 패턴 목록의 첫 패턴 시작 | 구현됨 |
| 락온 유지 1/2.5/4.5/7/10초 | 성공 락 1/2/3/4/5개 획득 | 구현됨 |
| 보스 HP 66.67% 이하 | Angry 대사 HUD를 3초 표시 | 구현됨 |
| 보스 HP 65% 이하 | `Tracking Residual Beam`이 라운드로빈 선택 대상에 추가 | 구현됨 |
| 보스 HP 33.33% 이하 | Normal 대사 HUD를 3초 표시 | 구현됨 |
| 보스 HP 0 | 보스 공격·플레이어 전투·이동을 중지하고 `R` 재시작 안내. 승리 오버레이/메인 메뉴 버튼 없음 | 구현됨 / 승리 UI 미구현 |
| 플레이어 Hull 0 | 보스 공격·플레이어 전투·이동을 중지하고 Mission Failed Retry/Quit 오버레이 표시 | 구현됨 |

`EventBroadcastHud`는 Normal/Surprise 프레임과 8 FPS 재생 View가 씬에 존재하지만 `ShowNormal`/`ShowSurprise`를 호출하는 런타임 코드가 없다. 따라서 첫 보스 공격이나 5락 완료 때 자동으로 나타나지 않는다.
...
| 행동 | PC 입력 | 모바일/HUD | 비고 |
| --- | --- | --- | --- |
| 수평 이동 | `A/D`, 좌/우 방향키 | 현재 별도 입력 없음 | 기본 7.2 월드 단위/초 / 락온 충전 중 4.32 월드 단위/초 |
| 수직 이동 | `W/S`, 위/아래 방향키 | 현재 별도 입력 없음 | 기본 7.2 월드 단위/초 / 락온 충전 중 4.32 월드 단위/초 |
...
```text
현재 기본 이동 속도 = 이전 기본값 8 월드 단위/초 × 0.9배 = 7.2 월드 단위/초
락온 충전 중 이동 속도 = 7.2 월드 단위/초 × 0.6배 = 4.32 월드 단위/초
```
...
| 자동 휴지 구간 | 2초 | 원본값 |
| 전체 반복 주기 | 4초 | 계산값 |
| 발사 구간 초당 발사 수 | 약 6.667발/초 | 계산값 |
| 발사 구간 이론 피해량 | 20 피해/초 | 계산값, 전탄 명중 기준 |
| 4초 주기 이론 평균 피해량 | 10 피해/초 | 계산값, 50% 발사 비율·전탄 명중 기준 |
| 탄속 | 60 월드 단위/초 | 원본값 |
| 탄 수명 | 4초 | 원본값 |
...
| 원본 실행 프로필 | 최소 충전 시간 | 발사 수 | 총 기본 피해량 | 1발 기본 피해량 |
| ---: | ---: | ---: | ---: | ---: |
| 1락 | 1.00초 | 5발 | 9 피해 | 1.8 피해/발 |
| 2락 | 2.50초 | 10발 | 20 피해 | 2 피해/발 |
| 3락 | 4.50초 | 15발 | 35 피해 | 약 2.333 피해/발 |
| 4락 | 7.00초 | 20발 | 60 피해 | 3 피해/발 |
| 5락 | 10.00초 | 30발 | 100 피해 | 약 3.333 피해/발 |
...
| ID | 표시/부위 | 기본 가중치 점수 | 속성 | 현재 기본 상태 |
| --- | --- | ---: | --- | --- |
...
| 연출용 Sidewinder 비행값 | 분리 속도 5 월드 단위/초를 0.5초 유지 → 가속도 20 월드 단위/초²로 순항 35 월드 단위/초까지 상승 / 회전 속도 180°/s |
| 발사대 | 좌/우 발사대 교대 |
| 풀 용량 | 40발 고정 풀, 현재 최대 요청 30발 |
| 전투 미사일 외형 | 검은 원통 몸체 + 진행 방향을 향한 원뿔형 기수. 기존 Sidewinder 복제 외형은 사용하지 않음 |
| 분산 전개 | 0.28초, 전개 거리 5.5 월드 단위 |
| 분산 전개 방향 계수 | 수평 기본 방향의 1.0배 / 수직 기본 방향의 0.65배 |
| 아크 비행 | 0.75초 ± 0.18초 |
| 아크 반경 | 수평 10 / 수직 7 월드 단위 |
| 발사 속도 | 18 월드 단위/초 |
| 순항 속도 | 72 월드 단위/초 |
| 가속도 | 130 월드 단위/초² |
| 회전 속도 | 280°/s |
...
목표점에는 수평 반경 1.6 월드 단위, 수직 방향 배율 1.25배, 깊이 분산 0.2 월드 단위를 적용해 동일 지점에 모든 미사일이 겹치지 않게 한다.
...
- 분리된 두 발은 락온 스냅샷의 앞쪽 두 타깃을 향해 동시에 유도 비행한다. 처음 0.5초는 5 월드 단위/초로 천천히 진행하고, 이후 20 월드 단위/초²로 가속해 최대 순항 속도 35 월드 단위/초에 도달한다. 회전 속도는 180°/s다. 전용 오버레이 렌더러에 외부 시각 루트로 등록하므로 건물이나 월드 지형의 깊이에 가려지지 않는다.
- 이 두 발은 `BattleController.TryHitBoss`나 다른 피해 API를 호출하지 않는 순수 시각 연출이다. 전투 풀 40발, 원본 단계별 전투 미사일 수 `5/10/15/20/30발`, 5락 실행 프로필 총 기본 피해 100에 포함되지 않는다.
- 각 Sidewinder는 타깃 반경 1.8 월드 단위 안에 도달하면 즉시 자기 원래 파일런의 로컬 위치·회전·스케일로 복귀한다. 타깃이 사라지면 릴리즈 때 저장한 위치로 계속 날아가며, 명중하지 못해도 분리 후 최대 6초에 강제 복귀한다.
...
- 실제 1~4락은 일제사격 무적과 전투 미사일 발사를 그대로 사용하지만 Base 카메라 투영 진동을 시작하지 않는다. 실제 5락 릴리즈가 성공해 풀살보 이벤트가 발생한 때만 진동을 시작하고, 헬기 회전과 장착 Sidewinder 분사 동안 유지한 뒤 두 대형 Sidewinder가 분리되어 `SalvoInvincibilityEnded`가 발생하는 같은 프레임에 원래 투영을 복구한다. 5락 기본 진폭은 투영행렬 정규화 오프셋 `0.0075`이며 현재 육안 판별용 **프로토타입/디버그** 상수 `TemporaryCameraShakeVisibilityTestMultiplier = 8배`를 적용해 유효 진폭은 정규화 오프셋 `0.0600`이다. Full HD 이론상 최대 이동은 가로 57.6px·세로 32.4px이고, 최신 BattleArena 5락 고정 월드 지점 관측 피크는 53.84px였다. 육안 확인이 끝나면 배율을 `1배`로 되돌릴 수 있다.
...
| 고정 미사일 피해 150 피해/발 | 현재 락온 피해 공식에서 사용하지 않음 |
...
| 항목 | 현재값 |
| --- | ---: |
| 최대 Hull | 100 Hull 포인트 |
| 최대 Armor | 120 Armor 포인트 |
...
| --- | ---: | --- |
| Hull | 레벨당 +10 Hull 포인트 | 에디터/개발 빌드 튜닝 적용 경로 |
| Armor | 레벨당 +10 Armor 포인트 | 에디터/개발 빌드 튜닝 적용 경로 |
| 기관총 피해 | 레벨당 +2 피해/발 | 개틀링에만 적용되며 락온 미사일 고정 피해표에는 영향 없음 |
| 미사일 피해 | 레벨당 +10 피해/발 | 현재 락온 미사일 경로에서 미사용 |
...
| 항목 | 현재값/규칙 |
| --- | --- |
| 보스 체력 | Single 2,000 HP / MultiPlaceholder 2,800 HP |
| 기본 투사체 피해 | 15 피해/발 |
| 기본 투사체 속도 | 24 월드 단위/초 |
| 투사체 전역 크기 배율 | 원본 크기의 2.5배 (`BossAttackController`) |
| 빔/위험 영역 전역 크기 배율 | 원본 폭·길이의 2.5배 (`BossBulletPatternController`) |
...
**화면에서 보이는 형태:** 보스가 꼬리를 내리찍은 뒤 `TailImpact` 한 지점에서 작은 파편 7~10개가 부채꼴 소나기처럼 플레이어 화면 평면을 향해 직선으로 퍼진다. 파편마다 서로 다른 메시를 사용할 수 있고 초당 260~720도로 회전하며 0.32초짜리 노랑→주황→빨강 Trail을 남긴다. 3~4개는 플레이어 근처 좁은 군집에 몰리고 나머지는 넓게 흩어진다.

**플레이어가 읽는 법:** Tail 동작과 경고선을 보고 밀집 군집의 중심에서 먼저 벗어난 뒤, 외곽으로 흩어진 파편 사이의 빈 공간을 확인한다. 작은 파편이 동시에 화면 전체를 채우는 전통 탄막이라기보다 한 충돌점에서 잔해가 터져 나오는 산탄형 공격이다.

| 항목 | 현재값 |
| --- | ---: |
| 발사 수 | 7~10개 |
| 발사 간격 | 0.025초 |
| 1개 피해 | 5.25 피해/개 (`15 피해 × 0.35배`) |
| 이동 속도 | 6 월드 단위/초 (`24 월드 단위/초 × 0.25배`) |
| 패턴 투사체 스케일 | 원본 크기의 0.45배 |
| 전역 투사체 스케일 | 패턴 크기의 2.5배 |
| 최종 생성 스케일 | 원본 크기의 1.125배 (`0.45 × 2.5`) |
| 패턴 후 쿨다운 배율 | 기본 공격 간격의 0.9배 |
| 현재 카이주 준비/경고 | Tail 애니메이션 릴리즈 큐 `48/30초 = 1.6초`까지 대기 |
| 무 AnimationDriver 폴백 | 0.2초 스톰프 2회 |

행동:

- 현재 BattleArena의 카이주 애니메이션 경로는 `TailImpact`에서 파편을 발생시키고 Tail 클립의 1.6초 릴리즈 큐까지 경고를 유지한다. 씬의 `telegraphDuration=0.25`는 이 경로의 실제 대기 시간을 결정하지 않는다.
- `debrisFragmentStompCount=2`, `debrisFragmentStompDuration=0.2`는 `KaijuBossAnimationDriver`가 없을 때만 사용하는 폴백이다. 현재 씬에서는 실행되지 않는다.
- 현재 카이주 경로는 릴리즈 때 0.1초·진폭 0.24의 작은 카메라 흔들림을 한 번 시작한다.
- 3~4개는 플레이어 근처의 한 군집을 겨냥하고 나머지는 더 넓게 흩어진다.
- 군집 반경은 기본 플레이어 피격 반경 기준 약 1.4 월드 단위, 전체 산포는 최대 약 8.4 월드 단위다.
...
**화면에서 보이는 형태:** 보스 입 쪽 발사점에서 대형 투사체 2~4발을 동시에 퍼뜨리지 않고 한 발씩 연속으로 던진다. 첫 발은 얇은 주황색 직선 경고로 목표를 보여주며, 각 투사체는 멀리서 작게 시작해 접근할수록 크게 보이고 초기 속도 6에서 가속한다. 패턴 이름과 역할은 파편이지만 현재 `BossProjectile.prefab`의 실제 렌더 외형은 주황색 재질의 Sphere 메시이므로 화면에서는 큰 주황색 구체탄이 차례로 돌진하는 모습이다.

**플레이어가 읽는 법:** 첫 경고 지점을 벗어난 뒤에도 계속 이동해야 한다. 후속 발은 발사 순간마다 플레이어 주변의 새 임의 지점을 겨냥하므로 첫 발을 피한 자리로 즉시 돌아가거나 정지하면 위험하다.
...
| 첫 발 경고 | 0.3초 |
| 발사 간격 | 0.2초 |
| 1발 피해 | 23.25 피해/발 (`15 피해 × 1.55배`) |
| 현재 보이는 외형 | 주황색 Sphere 메시 `BossProjectile` |
| 초기 속도 | 6 월드 단위/초, 이후 목표 지점에 1초 내 도달하도록 가속 |
| 조준 산포 반경 | 약 2.8 월드 단위 |
| 패턴 투사체 스케일 | 원본 크기의 2.8배 |
| 전역 적용 후 생성 스케일 | 원본 크기의 7.0배 |
| 접근 중 크기 보정 | 최종 생성 크기의 0.25배 → 1.19배 |
| 패턴 후 쿨다운 배율 | 기본 공격 간격의 1.05배 |

행동:

- 첫 발만 0.3초 경고한 뒤 주황색 구체형 대형 투사체를 순차 발사한다.
- 투사체는 플레이어 현재 위치 주변의 임의 지점을 겨냥한다.
...
**화면에서 보이는 형태:** 보스 입을 회전축으로 하는 길고 굵은 붉은 막대형 레이저다. 얇은 경고 빔이 화면 한쪽 끝을 가리킨 뒤, 실제 빔이 플레이어가 있던 화면 높이를 따라 반대쪽 끝까지 자동차 와이퍼처럼 좌→우 또는 우→좌로 한 번 쓸고 지나간다. 처음 20%는 느리지만 남은 80%에서 급가속하고, 이때 색이 더 붉어지며 폭도 최대 1.18배까지 넓어진다.

**플레이어가 읽는 법:** 경고 중에 진행 방향과 수평 라인을 읽고 위나 아래로 충분히 벗어난다. 후반 0.2초가 매우 빠르므로 빔을 따라 수평 이동하거나 마지막 순간에 넘는 방식은 불리하다.
...
| 빠른 구간 | 나머지 80%, 0.2초 |
| 총 횡단 시간 | 0.5초 |
| 1회 피해 | 18 피해/피격 (`15 피해 × 1.2배`) |
| 기본 폭 × 전역 배율 | `0.78 × 2.5 = 1.95` 월드 단위 |
| 빠른 구간 폭 보정 | 최대 1.18배 |
| 길이/위험 반경 × 전역 배율 | `28 × 2.5 = 70` 월드 단위 |
| 패턴 후 쿨다운 배율 | 기본 공격 간격의 1.2배 |
...
**화면에서 보이는 형태:** 보스 입에 주황색 구형 에너지가 0.5초 동안 맥동한 뒤, 입에서 플레이어 쪽 추적 지점까지 붉은 막대형 빔이 연결된다. 끝점이 플레이어를 제한된 속도로 쫓아가므로 빔 전체가 보스 입을 축으로 계속 방향을 틀며 4초간 유지된다. 이름과 달리 현재는 지나간 바닥에 장판이나 잔류 궤적을 남기는 공격이 아니라 지속형 추적 레이저다.

**플레이어가 읽는 법:** 충전 구체가 보일 때 미리 움직이고, 발사 뒤에는 빔 진행과 직각에 가까운 방향으로 계속 이동해 추적 끝점과 거리를 벌린다. 화면 가장자리에 몰리지 않도록 중앙 쪽 회피 공간을 남기는 것이 중요하며, 접촉을 유지하면 0.2초마다 피해가 반복된다.

| 항목 | 현재값 |
| --- | ---: |
| 충전/준비 시간 | 0.5초 |
| 추적 지속 시간 | 4초 |
| 추적 속도 | 플레이어 기준 최대 이동 속도의 40%, 현재 보통 4 월드 단위/초 |
| 연속 피해 | 3 피해/틱 |
| 피해 간격 | 0.2초 |
| 이론상 DPS | 15 피해/초 |
| 4초 최대 원시 피해 | 60 피해 |
| 기본 폭 × 전역 배율 | `0.62 × 2.5 = 1.55` 월드 단위 |
| 길이 | 70 월드 단위 |
| 패턴 후 쿨다운 배율 | 기본 공격 간격의 1.35배 |
...
| 피드백 | 현재 역할 | 상태 |
| --- | --- | --- |
| Player Hull / Armor | 현재 생존 상태와 Armor 파손/수리 상태 표시 | 구현됨 |
| Boss HP | 보스 잔여 체력 표시 | 구현됨 |
| 기관총 탄/피해 연출 | 2초 발사/2초 휴지 자동 주기, 발사체와 보스 피해 숫자 표시, 현재 크리티컬 표기 없음 | 구현됨 |
| Lock-on Button | 준비, 충전, 놓기, 쿨다운, 타깃 없음 상태 표시 | 구현됨 |
| Charge Gauge | 충전 진행과 단계 표시 | 구현됨 |
| World Reticle | 현재 충전 타깃은 점멸·스케일 왕복, 완료 타깃은 고정 표시하며 발사 후 유지 상태도 표시 | 구현됨 |
| Telegraph | 보스 빔/대형 공격의 사전 경고 | 구현됨 |
| Event Dialogue HUD | 보스 HP 66.67%에서 Angry, 33.33%에서 Normal 프레임을 각각 3초 표시 | 구현됨 |
| Event Broadcast HUD | Normal/Surprise 프레임과 8 FPS View는 있으나 런타임 호출자가 없음 | 프로토타입/미연결 |
| Music / Sound Buttons | 좌하단에서 전역 BGM과 BGM 외 효과음을 각각 독립적으로 ON/OFF. 둘 다 새 앱/Play Mode 시작 기본 OFF. 버튼보다 큰 공용 검은 배경과 입력 차단 없음 | 구현됨 |
| Result Overlay | 패배에만 Mission Failed + Retry/Quit 제공. 승리는 상태 문구 + `R`만 제공 | 부분 구현 |

현재 씬 안의 직렬화된 오래된 조작 안내 문자열에는 `Space / Left click fire`, `Missile button`, `Special button` 문구가 남아 있다. 실행 시 `HUDPresenter`가 자동 기관총/LOCK ON 안내로 덮어쓰므로 플레이 중 표시 기준은 최신이지만, 씬 원본 문자열은 UI 정리 대상이다.
...
| 보스 HP 대사 HUD | `Assets/_Project/Scripts/UI/BattleEventDialogueHealthTrigger.cs` | `BattleEventDialogueView.cs`, BattleArena의 `EventDialogueHud` 직렬화 임계값 |
| 방송 HUD | `Assets/_Project/Scripts/UI/BattleEventBroadcastView.cs` | BattleArena의 `EventBroadcastHud`; 현재 호출/이벤트 구독 코드 없음 |
...
| --- | --- | --- | --- |
| 높음 | 임시 화면 진동 ×8 배율 | 화면 진동은 실제 5락 풀살보에만 적용한다. 육안 판별을 위해 투영행렬 정규화 오프셋 `0.0075`에 `TemporaryCameraShakeVisibilityTestMultiplier = 8배`를 적용하며 유효 오프셋은 `0.0600`, Full HD 최신 실측 피크는 53.84px다 | 화면 진동 경로 확인이 끝나면 배율을 `1배`로 복원하고 최종 연출 진폭을 플레이 테스트로 결정 |
| 높음 | 약점 개방 게임플레이 | 약점 타깃과 2배 피해는 구현됐지만 개방 조건은 디버그 전용 | 체력 구간, 특정 패턴 후, 부위 파괴 등 실제 개방 규칙 결정 및 연결 |
| 높음 | 보스 패턴 코드 폴백과 씬 목록 차이 | 현재 BattleArena 직렬화 목록은 4종이라 Pressure Sniper가 비활성이다. 하지만 코드가 빈/구형 목록을 재생성할 때 쓰는 기본 `KaijuHeavyThreats`는 Pressure Sniper를 세 번째에 포함한 활성 5종 목록이다 | 4종을 공식 기본값으로 확정해 코드 폴백에서도 제거하거나, 5종 복귀 의도를 명시하고 씬과 문서를 함께 변경 |
...
| 중간 | 방송 HUD 이벤트 연결 | Normal/Surprise 프레임과 View는 구현됐지만 `ShowNormal`/`ShowSurprise` 런타임 호출자가 없어 항상 숨김 | 첫 보스 공격, 실제 5락 완료 등 원하는 트리거를 확정하고 명시적으로 구독/호출 |
| 중간 | 승리 결과 UX | 승리 시 상태 문구와 `R` 재시작만 있고 패배처럼 메인 메뉴 복귀 버튼이 있는 결과 오버레이는 없다 | 현 상태를 의도된 최소 흐름으로 유지할지, 승리 Retry/Quit/Next Stage UI를 추가할지 결정 |
...
### 2026-09-10 — BattleArena 전투 전 과정 정적 재검증

- Git `4a26b8e`에서 `BattleController`, 플레이어 이동/전투/락온/미사일, 보스 공통 공격/패턴, HUD 이벤트 코드를 현재 `BattleArena` YAML, Player/Boss Prefab, `VehiclePlayerStateCatalog.asset`과 교차검증했다.
- 현재 기본 메뉴는 `StageStepSelectScene`으로 직접 이동한다. 지역·난이도 선택값은 BattleArena HUD에 표시되지만 전투 수치·패턴·배경을 바꾸지 않는다. `MultiPlaceholder`는 메인 메뉴 `M` 키로만 접근하는 프로토타입이다.
- BattleArena의 직렬화된 `KaijuHeavyThreats`는 `Debris Fragment Scatter → Debris Salvo → Accelerating Sweep Beam → Tracking Residual Beam` 4종이다. 코드의 재생성용 기본 목록에는 Pressure Sniper가 포함된 5종이어서 결정 필요 불일치로 기록했다.
- 파편 산탄은 현재 `KaijuBossAnimationDriver`가 존재하므로 Tail 클립 릴리즈 큐 `48/30초=1.6초`를 기다린다. `0.2초 × 2회` 스톰프는 AnimationDriver가 없는 폴백 경로다.
- 파편 산탄의 회전 메시·0.32초 Trail, 파편 일제사격의 주황색 Sphere 프리팹·접근 스케일·가속, 두 빔의 Cube 기반 막대형 VFX와 화면 이동 공식을 확인해 네 패턴의 시각적 형태와 회피 포인트를 문서화했다.
- 세 현재 문서의 전투 수치 표를 재검토해 이동·투사체·추적 속도는 월드 단위/초, 가속도는 월드 단위/초², 거리·폭·반경은 월드 단위, 피해는 피해/발·개·틱·초, 생존값은 HP/Hull/Armor 포인트로 명시했다. 배율과 정규화 투영 진폭도 기준값을 구분했다.
- `EventDialogueHud`의 66.67%/33.33% 트리거는 런타임 연결되어 있다. `EventBroadcastHud`는 View와 프레임 에셋만 있고 호출자가 없으므로 전투에서 표시되지 않는다.
- 승리는 별도 결과 오버레이 없이 `R` 재시작만, 패배는 Retry/Quit 오버레이를 사용한다.
- 이번 작업은 문서 정합성 정적 감사이며 Unity PlayMode, EditMode Test Runner, 실제 빌드는 재실행하지 않았다. 기존 2026-08-31 이하의 런타임 검증 기록은 이력으로 유지한다.
...
| 날짜 | 버전 | 변경 내용 | 검증 |
| --- | --- | --- | --- |
| 2026-09-10 | 1.0.46 | 세 현재 문서의 숫자만 표기된 전투값에 속도·가속도·거리·폭·반경·피해·HP·배율 단위와 기준을 추가 | 현재 전투 수치 표·공식 정적 검토, `git diff --check`; PlayMode 미실행 |
| 2026-09-10 | 1.0.45 | 활성 보스 4패턴이 화면에서 보이는 모양, 생성 위치, 이동 방식, 회피 포인트를 코드 기준으로 보강 | `BossBulletPatternController`, `DebrisFragmentProjectileVisualRuntime`, `BossProjectile.prefab`, `KaijuBossAnimationDriver` 정적 확인; PlayMode 미실행 |
| 2026-09-10 | 1.0.44 | BattleArena 시작부터 종료까지 C#·씬·Prefab·SO 정적 재대조. 현재 4패턴, Tail 기반 파편 산탄 준비, 지역/난이도 미연동, 방송 HUD 미연결, 승리/패배 흐름을 현재 구현 기준으로 정정 | Git `4a26b8e` 정적 교차검증, `git diff --check`; PlayMode/빌드 미실행 |
